...
const JobListing = () => {
    ...
    useEffect(()=>{
        ...
        const matchesLocation = job => selectLocations.length === 0 || selectLocations.includes(job.location)
        ...
        setCurrentPage(1)
    },[jobs , selectedCategories , selectLocations , searchFilter])
    ...
    return (
        <div className='container 2xl:px-20 mx-auto flex flex-col lg:flex-row max-lg:space-y-8 py-8'>
            ...
            <div className='w-full lg:w-1/4 bg-white px-4'>
                ...
                </button> {/*this effect is for moile view */}
                    ...
                    <div className='flex items-center justify-center space-x-2 mt-10 '>
                        ...
                        {Array.from({ length: Math.ceil(filteredJobs.length / 6) }).map((_, index) => (
                            <a key={index} href='#job-list'>
                                <button onClick={() => setCurrentPage(index + 1)} className={`w-10 h-10 flex items-center justify-center border border-gray-300 rounded ${currentPage === index + 1 ? 'bg-blue-200 text-blue-500' : 'text-gray-500'}`}>{index + 1}</button>
                            </a>
                        ))}
                        <a href='#job-list'>
